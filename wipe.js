...
function hideLoadingOverlay() {
  byId('loading-overlay').style.opacity = 0;
  setTimeout(function() {
...
document.addEventListener('touchend', function(e) {
  ...
  if ((now - touchStartTime < MAX_ANIMATION_TOUCH_MOVE_TIME && Math.abs(offsetY) > 50) ||
    ...
    if (!prevPage || prevPage.id == 'index') {
      prevPage = document.querySelector('.page14')
    }
    ...
    setTimeout(function() {
      ...
      // 关掉视频，恢复音乐
      if (prevVisiblePage.id == 'page10') {
        videoPlaying = false;
        video.pause();
        video.style.display = 'none'
        if (!musicPlaying && videoPauseMusic) {
          music.play()
          musicPlaying = true
          videoPauseMusic = false
        }
      } else if (prevVisiblePage.id == 'page11') {
        stopPhotoChangeAnimation();
      }
      ...
      switch (currentPage.id) {
        case 'page2':
          if (!hasClass(currentPage.querySelector('.page2-text'), 'fadein')) {
            currentPage.querySelector('.page2-text').classList.add('fadein');
          }
          break;
        case 'page3':
          // if (!hasClass(currentPage.querySelector('.page3-text'), 'fadein')) {
            currentPage.querySelector('.page3-cloud').classList.add('page3-cloud-animation')
            currentPage.querySelector('.page3-bg').classList.add('fadein')
          // }
          break;
        case 'page4':
          currentPage.querySelector('.page4-bg-cover').classList.add('fadein')
          // currentPage.querySelector('.page4-text-1').classList.add('page4-text1-animation')
          // currentPage.querySelector('.page4-text-2').classList.add('page4-text2-animation')
          // currentPage.querySelector('.page4-line1').classList.add('fadein')
          // currentPage.querySelector('.page4-line2').classList.add('fadein')
          break;
        case 'page5':
          currentPage.querySelector('.page5-text1').classList.add('page5-text1-animation')
          currentPage.querySelector('.page5-text2').classList.add('page5-text2-animation')
          currentPage.querySelector('.page5-line1').classList.add('fadein')
          break;
        case 'page6':
          currentPage.querySelector('.page6-text1').classList.add('fadein')
          break;
        case 'page7':
          // currentPage.querySelector('.page7-text1').classList.add('page7-text1-animation')
          // currentPage.querySelector('.page7-text2').classList.add('page7-text2-animation')
          // currentPage.querySelector('.page7-line1').classList.add('fadein')
          currentPage.querySelector('.page7-text').classList.add('fadein')
          break;
        case 'page8':
          // currentPage.querySelector('.page8-text1').classList.add('page8-text-animation')
          // currentPage.querySelector('.page8-line1').classList.add('fadein')
          currentPage.querySelector('.page8-text').classList.add('fadein')
          break;
        case 'page9':
          currentPage.querySelector('.page9-text').classList.add('fadein')
          break;
        case 'page11':
          runPhotoChangeAnimation()
          break;
        case 'page12':
          currentPage.querySelector('.page12-text').classList.add('fadein')
          break;
      }
    }, 300)
  } else {
...
var progressWrap = byId('progress-wrap')
...
var video = byId('page10-video')
var videoPlayBtn = byId('video-play-btn')
var videoPauseMusic = false;
var videoPlaying = false;
videoPlayBtn.addEventListener('click', function() {
  videoPlaying = true;
  if (musicPlaying) {
    videoPauseMusic = true;
    music.pause();
    musicPlaying = false;
  }
...
var photosCache = {};
for (var i = 1; i < 14; i++) {
  photosCache['photo' + i] = new Image()
  photosCache['photo' + i].src = './images/photos/' + i + '.jpg'
}

var photosWrap = byId('page11-photos')
var currentPhotoIndex = 1;
var photoChangeTimer;
function runPhotoChangeAnimation() {
  var prevPhoto = photosCache['photo' + currentPhotoIndex]
  var currentPhoto
  photosWrap.innerHTML = ''
  photosWrap.appendChild(prevPhoto)

  photoChangeTimer = setInterval(function() {
    var index = currentPhotoIndex++ + 1
    if (index > 13) {
      index = 1
      currentPhotoIndex = 1
    }
    currentPhoto = photosCache['photo' + index]
    photosWrap.insertBefore(currentPhoto, photosWrap.lastElementChild)

    prevPhoto.style.opacity = 0;
    setTimeout(function() {
      photosWrap.removeChild(prevPhoto)
      prevPhoto.style.opacity = 1;
      prevPhoto = currentPhoto
    }, 1000)
  }, 5000)
}

function stopPhotoChangeAnimation() {
  clearInterval(photoChangeTimer)
  currentPhotoIndex = 1;
  photosWrap.innerHTML = ''
}

byId('follow-btn').addEventListener('click', function() {
  viewProfile()
}, false)
...
function hasClass(elem, _class) {
  return new RegExp('\\b' + _class + '\\b').test(elem.className)
}